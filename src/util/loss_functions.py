...
class MeanSquaredError(Error):
    ...
    def calculateError(self, target, output):
        ...
        return (1.0/len(target)) * reduce(lambda x, y: x + y, map(lambda pair: pow(pair[0] - pair[1], 2), zip(target,output)))
...
class SumSquaredError(Error):
    ...
    def calculateError(self, target, output):
        ...
        return 0.5*reduce(lambda x, y: x + y, map(lambda pair: pow(pair[0] - pair[1], 2), zip(target,output)))
